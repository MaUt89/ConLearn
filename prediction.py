...
from previous_prediction import write_next_possible_value
...
def prediction(settings):
    ...
    label_Names, features_Data, prediction_names = xml_parse_categories(settings["PROGRESS_XML_FILE_PATH"],
                                                      settings["IRRELEVANT_FEATURES"], settings["CATEGORIES_FILE_PATH"])
    if not label_Names:
        return 0

    # previous_prediction = check_previous_prediction(label_Names, settings["PREDICTION_REST_FILE_PATH"])
    previous_prediction = check_previous_prediction(prediction_names, settings["PREDICTION_REST_FILE_PATH"])
    if previous_prediction:
        prediction_values = write_next_possible_value(previous_prediction, settings["PREDICTION_REST_FILE_PATH"])
    ...
    # prediction_values = ConLearn.model_predict(id, test_Input, label_Names, label_Dict,
                                               # settings["PREDICTION_REST_FILE_PATH"])
    prediction_values = ConLearn.model_predict_cluster(id, test_Input, label_Names, label_Dict, prediction_names,
                                                       settings["PREDICTION_REST_FILE_PATH"])
    xml_write(prediction_values, settings["PROGRESS_XML_FILE_PATH"], settings["OUTPUT_XML_FILE_PATH"])
